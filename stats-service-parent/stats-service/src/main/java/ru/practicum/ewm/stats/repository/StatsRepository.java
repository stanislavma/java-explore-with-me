...
public interface StatsRepository extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("SELECT new ru.practicum.ewm.stats.model.EndpointHit(s.app, s.uri, COUNT(s)) " +
    ...
    @Query("SELECT new ru.practicum.ewm.stats.model.EndpointHit(s.app, s.uri, COUNT(DISTINCT s.ip)) " +
            "FROM EndpointHit s WHERE s.hitDate BETWEEN :start AND :end " +
            "GROUP BY s.app, s.uri " +
            "ORDER BY COUNT(DISTINCT s.ip) DESC")
    List<EndpointHit> findUniqueStatsWithoutUris(LocalDateTime start, LocalDateTime end);

    @Query("SELECT new ru.practicum.ewm.stats.model.EndpointHit(s.app, s.uri, COUNT(s.id)) " +
            "FROM EndpointHit s WHERE s.hitDate BETWEEN :start AND :end " +
